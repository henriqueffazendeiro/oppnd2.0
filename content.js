// Gmail Read Ticks — versão estável com polling (sem SSE)
console.log('Gmail Ticks: content script (polling) carregado');
...
let readByTrackingId = new Set(); // IDs já lidos
...
function getWhoFromRow(row) {
  ...
  // procurar noutros nós
  const any = row.querySelector('[data-hovercard-id],[email],[title]');
  if (any) {
    const em = extractEmail(any.getAttribute('data-hovercard-id') || any.getAttribute('email') || any.getAttribute('title') || '');
    if (em) return em.toLowerCase();
  }
  // texto
  const raw = (span.textContent || '').trim();
  const emt = extractEmail(raw); if (emt) return emt.toLowerCase();
...
function ensureTickContainer(row) {
  const cell = row.querySelector('td.xW') || row.querySelector('td:last-child') || row.lastElementChild;
...
function handleSend(composeRoot) {
  ...
  img.src = `${API_BASE}/track/${id}`;
  img.width = 1; img.height = 1; img.style.display = 'none';
  editor.appendChild(img);

  // candidatos para matching robusto
  ...
  // (opcional) registar meta (não é obrigatório)
...
// Observar mudanças da inbox
const mo = new MutationObserver(() => scheduleAddTicks(200));
mo.observe(document.documentElement, { childList:true, subtree:true });

// Primeira passagem
scheduleAddTicks(800);
console.log('Gmail Ticks: pronto (polling)');

// Poll global — consulta TODOS os ids pendentes a cada 5s (até 50/loop)
...
setInterval(async () => {
  if (!extensionActive) return;
  const ids = allPendingIds().slice(0, 50);
